...
router.get('/browse/:id', function(req, res) {
    var db = req.db;
    var collection = db.get('browse');
    var entryToFind = req.params.id;
    collection.find({ '_id' : entryToFind },{},function(e,docs){
        res.json(docs);
    });
});
...
router.get('/tradeoffers/:id', function(req, res) {
    var db = req.db;
    var collection = db.get('tradeoffers');
    var entryToFind = req.params.id;
    collection.find({ '_id' : entryToFind },{},function(e,docs){
...
/*
 * DELETE to delete entry Trade Offers
 */
router.delete('/deletefromtradeoffers/:id', function(req, res) {
    var db = req.db;
    var collection = db.get('tradeoffers');
    var entryToDelete = req.params.id;
    collection.remove({ '_id' : entryToDelete }, function(err) {
